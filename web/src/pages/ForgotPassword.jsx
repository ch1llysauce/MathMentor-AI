  ...
  return (
    <div className="min-h-screen bg-[#f7f9fb] flex flex-col justify-center items-center px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-[#e0e3e5] p-8">
          {success ? (
            <div className="text-center py-4">
              ...
              <h2 className="text-xl font-bold text-[#091426] mb-2">Password Reset!</h2>
              <p className="text-sm text-[#45474c] mb-6">{success}</p>
              <Link to="/login"
                className="inline-flex items-center gap-2 bg-[#4b41e1] text-white font-semibold px-6 py-3 rounded-xl hover:bg-[#3323cc] transition-colors">
                Back to Sign In
              </Link>
            </div>
          ) : (
            <>
              ...
              <div className="flex items-center justify-center gap-3 mb-6 max-w-xs mx-auto">
                {STEPS.map((s, i) => (
                  <div key={s} className={`flex items-center gap-3 ${i < STEPS.length - 1 ? 'flex-1' : ''}`}>
                    ...
                    {i < STEPS.length - 1 && (
                      <div className={`flex-1 h-0.5 rounded-full ${i + 1 < step ? 'bg-[#00a472]' : 'bg-[#f2f4f6]'}`} />
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className="mb-6">
                <h1 className="text-xl font-bold text-[#091426]">
              ...
              {step === 1 && (
                <form onSubmit={handleSendOtp} className="space-y-4">
                  ...
                  <button type="submit" disabled={loading}
                    className="w-full bg-[#4b41e1] text-white font-semibold py-3 rounded-xl hover:bg-[#3323cc] disabled:opacity-60 transition-colors mt-2">
              ...
              {step === 2 && (
                <form onSubmit={handleVerifyOtp} className="space-y-4">
                  <div>
                    <label className="block text-xs font-semibold text-[#45474c] uppercase tracking-wide mb-1.5 ml-1">OTP Code</label>
                  ...
                  <button type="button" onClick={() => { setStep(1); setError(''); }}
                    className="w-full flex items-center justify-center gap-1 text-sm text-[#75777d] hover:text-[#45474c] py-2">
        ...
        {!success && (
          <p className="text-center text-sm text-[#45474c] mt-6">
            Remembered it?{' '}
