// Comprehensive MathMentor AI Curriculum Structure
// This defines the complete curriculum hierarchy for all three subjects
...
export function getLesson(subject, moduleNumber, lessonOrder) {
  const module = CURRICULUM[subject].modules.find(m => m.moduleNumber === moduleNumber);
  if (!module) return null;
  return module.lessons.find(l => l.order === lessonOrder);
}

// Helper function to generate flat lesson list
export function getFlatLessonList(subject) {
  const lessons = [];
  CURRICULUM[subject].modules.forEach(module => {
    module.lessons.forEach(lesson => {
      lessons.push({
        ...
        fullPath: `${subject} → Module ${module.moduleNumber}: ${module.moduleName} → ${lesson.title}`
      });
    });
  });
  return lessons;
